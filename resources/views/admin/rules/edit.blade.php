    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="p-8 shadow bg-base-100 rounded-box">
                <form method="POST" action="{{ route('rules.update', $rule['_id']) }}" id="editRuleForm">
                    @csrf
                    @method('PUT')
                    <div class="mb-5">
                        ...
                        <x-default-input name="rules" value="{{ $rule['rules'] ?? null }}" />
                    </div>
                    <div class="mb-5">
                        <label for="content" class="label">
                            <span class="font-medium label-text">{{ __('Content') }}</span>
                        </label>
                        <x-default-textarea name="content" value="{{ $rule['content'] ?? null }}" />
                    </div>
                    <div class="mb-5">
                        <label class="label">
                            <span class="font-medium label-text">{{ __('Main Keywords') }}</span>
                        </label>
                        <!-- Hidden input will be populated by JS -->
                        ...
                        <div id="keywords-container" class="flex flex-wrap gap-2 p-2 border border-gray-300 border-dashed rounded-md min-h-10 bg-base-200">
                            @php
                            $keywords = old('main_keyword', $rule['main_keyword'] ?? []);
                            @endphp
    ...
    <script>
        $(document).ready(function() {
            ...
            function updateHiddenField() {
                ...
                $('input[name^="main_keyword"]').remove();
                keywords.forEach(function(keyword) {
                    $('<input>').attr({
                        type: 'hidden',
                        name: 'main_keyword[]',
                        value: keyword
                    }).appendTo('#editRuleForm');
                });
            }
            updateHiddenField();
        });
